import Reveal from "./Reveal";
import SectionHeading from "./SectionHeading";
import { skills, careers, sideWork } from "@/lib/data";

// 小項目の見出し（Experience セクション内のサブセクション用）
function SubHeading({ children }: { children: React.ReactNode }) {
  return (
    <h3 className="text-sm uppercase tracking-[0.2em] text-[var(--color-muted)]">
      {children}
    </h3>
  );
}

export default function Experience() {
  return (
    <section
      id="experience"
      className="mx-auto max-w-[var(--container)] px-6 py-24 md:py-36"
    >
      <SectionHeading no="03" title="Experience" />

      <div className="grid gap-16 md:grid-cols-2 md:gap-20">
        {/* 経歴（正社員 + 副業・業務委託） */}
        <div>
          {/* 正社員：年号を含む時系列 */}
          <Reveal>
            <SubHeading>Career</SubHeading>
          </Reveal>
          <ul className="mt-8 space-y-10">
            {careers.map((c, i) => (
              <Reveal as="li" key={c.company} delay={i * 80}>
                <p className="text-xs tracking-widest text-[var(--color-muted)]">
                  {c.period}
                </p>
                <p className="mt-1 text-lg font-semibold tracking-tight">
                  {c.company}
                </p>
                <p className="text-sm text-[var(--color-muted)]">{c.role}</p>
                <div className="mt-3 flex flex-wrap gap-2">
                  {c.stack.map((t) => (
                    <span
                      key={t}
                      className="rounded-full border border-[var(--color-line)] px-3 py-1 text-xs text-[var(--color-muted)]"
                    >
                      {t}
                    </span>
                  ))}
                </div>
              </Reveal>
            ))}
          </ul>

          {/* 副業・業務委託：期間表記なし、案件を箇条書き */}
          <div className="mt-14">
            <Reveal>
              <SubHeading>副業・業務委託（一部抜粋）</SubHeading>
            </Reveal>
            <ul className="mt-6 space-y-5">
              {sideWork.map((w, i) => (
                <Reveal as="li" key={w.name} delay={i * 80}>
                  <p className="text-base font-semibold tracking-tight">
                    {w.name}
                  </p>
                  <p className="mt-0.5 text-sm text-[var(--color-muted)]">
                    {w.description}
                  </p>
                  {w.url && (
                    <a
                      href={w.url}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="group mt-1 inline-flex w-fit items-center gap-1 break-all text-sm text-[var(--color-muted)] underline underline-offset-4 transition-colors hover:text-[var(--color-ink)]"
                    >
                      {w.url}
                      <svg
                        viewBox="0 0 24 24"
                        fill="none"
                        stroke="currentColor"
                        strokeWidth="1.5"
                        strokeLinecap="round"
                        strokeLinejoin="round"
                        className="h-3.5 w-3.5 shrink-0 transition-transform duration-200 group-hover:translate-x-0.5 group-hover:-translate-y-0.5"
                        aria-hidden="true"
                      >
                        <path d="M7 17 17 7M9 7h8v8" />
                      </svg>
                    </a>
                  )}
                </Reveal>
              ))}
            </ul>
          </div>
        </div>

        {/* Skills（小項目） */}
        <div>
          <Reveal>
            <SubHeading>Skills</SubHeading>
          </Reveal>
          <ul className="mt-8 space-y-6">
            {skills.map((s, i) => (
              <Reveal as="li" key={s.name} delay={i * 50}>
                <div className="flex items-baseline justify-between">
                  <span className="text-base font-medium">{s.name}</span>
                  <span className="text-xs tracking-widest text-[var(--color-muted)]">
                    {s.level}
                  </span>
                </div>
                <div className="mt-2 h-px w-full bg-[var(--color-line)]">
                  <div
                    className="h-px bg-[var(--color-ink)]"
                    style={{ width: `${s.level}%` }}
                  />
                </div>
              </Reveal>
            ))}
          </ul>
        </div>
      </div>
    </section>
  );
}
