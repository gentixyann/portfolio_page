...
`components/Contact.tsx` のフォームは [Web3Forms](https://web3forms.com/) 経由でメール送信します
（静的サイトのためサーバー不要）。利用には **Access Key** が必要です。

1. https://web3forms.com/ で送信先メール（`a551.return@gmail.com`）を登録し、Access Key を発行
2. プロジェクト直下に `.env.local` を作成して以下を記述（キーは公開しても安全な設計）

   ```
   NEXT_PUBLIC_WEB3FORMS_ACCESS_KEY=発行されたキー
   ```

   ※ `.env.local` を使わず、`components/Contact.tsx` の `ACCESS_KEY` を直接書き換えても可。
3. `npm run build` で反映。キー未設定だと送信は失敗します。
